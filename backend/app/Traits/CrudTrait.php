...
trait CrudTrait
{
     ...
     * @param  mixed $table String: Nombre de la Tabla
     * @return JSON
     */
    static function insert(Request $request, array $data, string $table)
    {
        if ($data && $table) {
            try {
                DB::beginTransaction();
    ...
    */
     ...
     * @return JSON
     */
    static function update(Request $request, $data = null, string $table)
    {
        if ($data && $table) {
            try {
                DB::beginTransaction();
    ...
    public static function audit($ip, $table, $what_did, $data)
    {
        // User
        $user = auth()->user();
        $audit  = [
