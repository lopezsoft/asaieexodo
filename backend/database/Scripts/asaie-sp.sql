...
BEGIN
		...
		tc.year, tmc.proc1, tmc.proc2, tmc.proc3, tmc.proc4, te.school_name NOMBRE_IE, RTRIM(tar.area) area,
...
-- Volcando estructura para procedimiento asaie_exodo.sp_electoral_certificate
...
-- Volcando estructura para procedimiento asaie_exodo.sp_select_consolidated_totas
DROP PROCEDURE IF EXISTS `sp_select_consolidated_totas`;
DELIMITER //
CREATE PROCEDURE `sp_select_consolidated_totas`(
	IN `_id_head` INT(20),
	IN `_id_study_day` INT(1),
	IN `_id_grade` INT(11),
	IN `_group` VARCHAR(2),
...
BEGIN
	IF _type = 1 THEN /* Asignaturas*/
		SET @tb 	= "consolidado_totales";
	ELSE
		SET @tb	= "consolidado_areas_totales";
	END IF;
	SET @sqlSelect = CONCAT("
		SELECT * FROM ",@tb," WHERE id_grado = ",_id_grade," AND id_sede=",_id_head,
                " AND year = ",_year," AND id_jorn=",_id_study_day," AND grupo= '",_group,"' AND periodo <> 0");
   PREPARE smtp FROM @sqlSelect;
   EXECUTE smtp;
   DEALLOCATE PREPARE smtp;
END//
	...
	CASE _type
		WHEN 3 THEN
			SELECT CONCAT(RTRIM(ti.apellido1),' ',RTRIM(ti.apellido2),' ',RTRIM(ti.nombre1),' ',RTRIM(ti.nombre2)) nombres,
				...
				tz.zone_name AS zona, tdoc.tipo as documento, tdoc.abrev AS abrev_doc, tg.grado,
				...
				TRIM(tg.grado) AS grado, nv.nombre_nivel, ti.foto AS avatar
				FROM inscripciones AS ti
			 	LEFT JOIN cities AS tc  ON (ti.lug_nacimiento = tc.id)
			 	LEFT JOIN departments AS td ON tc.id_dept = td.id
			 	LEFT JOIN cities AS tce  ON (ti.lug_expedicion = tce.id)
			 	LEFT JOIN departments AS tde ON tce.id = tde.id
			 	LEFT JOIN cities AS tcu  ON (ti.lug_residencia = tcu.id)
			 	LEFT JOIN departments AS tdu ON tcu.id_dept = tdu.id
				LEFT JOIN student_enrollment AS tm ON tm.id_student = ti.id
				JOIN jornadas AS tj ON tm.id_study_day=tj.cod_jorn
				JOIN sedes AS ts ON tm.id_headquarters=ts.id
				...
				JOIN niveles_estudio AS nv ON nv.id = tg.id_nivel
...
BEGIN
	...
	JOIN sedes AS ts ON tm.id_headquarters = ts.ID
	JOIN registration_status AS te on tm.id_state = te.id
	LEFT JOIN acta_promocion AS ta ON tm.id =  ta.id_matric
...
BEGIN
	...
	ELSE
		...
		ORDER BY tm.id_grado, au.id_area, tm.id_asig;
	END IF;
END//
DELIMITER ;
...
BEGIN
	SELECT tm.id,tm.id_group,tm.id_study_day,tm.id_headquarters, tm.id_grade,
		...
		rs.name_state AS estado
		FROM inscripciones AS te
		JOIN student_enrollment AS tm ON tm.id_student = te.id
		JOIN sedes AS ts ON tm.id_headquarters = ts.ID
		JOIN grados AS tg ON tm.id_grade = tg.id
		JOIN jornadas AS tj ON tm.id_study_day = tj.cod_jorn
		JOIN registration_status AS rs ON rs.id = tm.id_state
		WHERE tm.id_grade = _grade AND tm.id_study_day = _study_day
		AND tm.id_headquarters = _headq AND tm.year= _year
		AND tm.id_group = _group AND tm.id_state IN(2,4)
		ORDER BY nombres;
END//
DELIMITER ;
