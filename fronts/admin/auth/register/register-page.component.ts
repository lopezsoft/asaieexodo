...
import { CitiesService, DocumentsService } from './../../services/global/index';
...
export class RegisterPageComponent extends FormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    ts.documentSer.getTypeOrganization({}).subscribe((resp) => {
      ts.organizations = resp;
    });
    ts._cities.getData({}).subscribe((resp) => {
      ts.cities = resp;
      ts.hideSpinner();
    });
  }
